  ...
  render() {
    ...
    return (
      <div className={showReset ? "try-out btn-group" : "try-out"}>
        {
          enabled ? <button className="btn try-out__btn cancel" onClick={ onCancelClick }>Cancele</button>
                  : <button className="btn try-out__btn" onClick={ onTryoutClick }>Experimente </button>

        }
        {
          showReset && <button className="btn try-out__btn reset" onClick={ onResetClick }>Recomece</button>
        }
      </div>
    )
  }
